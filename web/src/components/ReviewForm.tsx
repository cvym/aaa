...
const ReviewForm = () => {
  ...
  return (
    <motion.div
      className="glass rounded-3xl p-6 text-sm"
      initial={{ opacity: 0, y: 24 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, amount: 0.3 }}
      transition={{ duration: 0.5 }}
    >
      <h3 className="font-heading text-xl font-semibold text-white">Leave a review</h3>
      ...
      <div className="mt-4">
        ...
        <input
          ...
          className="mt-2 w-full rounded-lg border border-white/15 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/40 focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/40"
        />
      </div>
      ...
      <div className="mt-5 flex items-center gap-3">
        <button
          ...
          className="rounded-full bg-gradient-to-r from-brand to-brand-accent px-5 py-2 text-sm font-semibold uppercase tracking-wide text-white shadow-[0_18px_40px_-22px_rgba(88,101,242,0.8)] transition hover:from-brand-accent hover:to-brand disabled:cursor-not-allowed disabled:opacity-70 disabled:shadow-none"
        >
